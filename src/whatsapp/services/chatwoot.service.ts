...
export class ChatwootService {
  ...
  private getAdsMessage(msg: any) {
    ...
    let adsMessage: AdsMessage | undefined;
    const adSrcMsg = msg.extendedTextMessage?.contextInfo.externalAdReply as AdsMessage;

    this.logger.verbose('Get ads message if it exist');
    if (adSrcMsg) {
      // Destructuring to get only the needed properties,
      // because the source original message have more properties.
      const { title, body, thumbnailUrl, sourceUrl } = adSrcMsg;
      adsMessage = {
        title,
        body,
        thumbnailUrl,
